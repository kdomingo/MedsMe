...
import com.example.medtracker.ui.util.Days
import com.example.medtracker.ui.util.formatMinutesOfDay
import com.example.medtracker.ui.viewmodel.MedDetailsViewModel
            ...
            Card(colors = CardDefaults.cardColors(containerColor = MaterialTheme.colorScheme.surface)) {
                Column(modifier = Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
                    ...
                    Row(verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.spacedBy(12.dp)) {
                        Button(onClick = { vm.takeDose(m.doseAmount) }) {
                            Text("Take ${m.doseAmount} ${m.doseUnit}")
                        }
                        OutlinedButton(onClick = { vm.takeDose(1.0) }) {
                            Text("Take 1")
                        }
                    }
                }
                ...
                Column(modifier = Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
                    ...
                    if (recent.isEmpty()) {
                        ...
                        recent.take(10).forEach { e ->
                            Text(
                                text = "- ${e.amount} at ${java.text.DateFormat.getDateTimeInstance().format(java.util.Date(e.takenAtEpochMs))}",
                                style = MaterialTheme.typography.bodyMedium,
                            )
                        }
                    }
